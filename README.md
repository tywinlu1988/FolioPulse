...
```
Step 1: 规则过滤         Step 2: 多因子打分        Step 3: 双轨验证        Step 4: 排序+理由
┌──────────────┐       ┌──────────────┐       ┌──────────────┐       ┌──────────────┐
│ 风险等级匹配   │       │ 股票 7 因子   │       │ 轨A 基本面    │       │ 综合评分降序   │
│ 投资期限匹配   │  →   │ 基金 7 因子   │  →   │ 轨B 市场信号  │  →   │ 每只 ≤3 条    │
│ 起投金额过滤   │       │ ETF 4 因子   │       │ 冲突裁决      │       │ 推荐理由      │
│ 禁售名单过滤   │       │ 加权求和      │       │               │       │              │
│ 客户约束过滤   │       │ 置信度评估    │       │               │       │              │
└──────────────┘       └──────────────┘       └──────────────┘       └──────────────┘
```
...
| 版本 | 里程碑 | 预计 |
|------|--------|------|
| **v0.1.0** ✅ | 三阶段管道跑通，单适配器（东方财富模拟），45 测试通过 | 已完成 |
| **v0.1.1** 🔄 | README 完善，npx 安装指引，引擎模块文档 | 进行中 |
| **v0.2.0** | 真实东方财富 API 接入，天天基金适配器，L0 CLI 格式化输出 | 计划中 |
| **v0.3.0** | RM 交互式调整（换产品/改配比/加备注），备选池功能 | 计划中 |
| **v0.4.0** | Mode B 外部数据（Wind），多适配器全覆盖，双轨验证实装 | 计划中 |
| **v0.5.0** | 跨 CLI 完整适配，端到端验证报告，真实产品数据库 | 计划中 |
